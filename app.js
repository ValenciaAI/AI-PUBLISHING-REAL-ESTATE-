...
function guideSpeak(text){if(!("speechSynthesis"in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang="ru-RU";u.rate=.96;u.pitch=1.08;u.onstart=()=>$("#guideStatus").textContent="Озвучиваю текущий статус";u.onend=()=>$("#guideStatus").textContent="Готова озвучивать миссию";window.speechSynthesis.speak(u)}
function answerGuide(heard){heard=String(heard||"").trim();if(!heard)return;$("#guideStatus").textContent="Команда: "+heard;guideSpeak(heard.toLowerCase().includes("привет")?"Привет! Я в порядке. Агенты в костюмах принимают исходные данные, расставляют фотографии по приоритету и готовят публикацию. Сейчас всё под контролем.":"Команда принята. Я проверяю исходные данные объекта, фотографии и статус агентов. О результатах сообщу сразу.")}
function startGuideVoice(){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){$("#guideStatus").textContent="Микрофон не поддерживается — введите команду ниже";return}const r=new Recognition();r.lang="ru-RU";r.continuous=false;r.interimResults=false;r.maxAlternatives=1;r.onstart=()=>$("#guideStatus").textContent="Слушаю вас…";r.onerror=e=>{$("#guideStatus").textContent=e.error==="not-allowed"?"Разрешите доступ к микрофону":"Не удалось услышать, нажмите ещё раз"};r.onend=()=>{if($("#guideStatus").textContent==="Слушаю вас…")$("#guideStatus").textContent="Готова озвучивать миссию"};r.onresult=e=>answerGuide(e.results[0][0].transcript);try{r.start()}catch{guideSpeak("Разрешите доступ к микрофону, чтобы я могла вас услышать.")}}
$("#voiceBtn").onclick=()=>{if($("#guideStatus").textContent.includes("Слушаю"))return;startGuideVoice()};$("#voiceText").onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();answerGuide(e.target.value);e.target.value=""}}
