...
function startGuideVoice(){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){guideSpeak("Голосовой режим доступен через кнопку озвучивания статуса. Ваш браузер не поддерживает распознавание речи.");return}const r=new Recognition();r.lang="ru-RU";r.interimResults=false;r.maxAlternatives=1;r.onstart=()=>$("#guideStatus").textContent="Слушаю вас…";r.onerror=()=>$("#guideStatus").textContent="Не удалось услышать, нажмите ещё раз";r.onresult=e=>{const heard=e.results[0][0].transcript.toLowerCase();$("#guideStatus").textContent="Вы сказали: "+heard;guideSpeak(heard.includes("привет")?"Привет! Я в порядке. Агенты в костюмах принимают исходные данные, расставляют фотографии по приоритету и готовят публикацию. Сейчас всё под контролем.":"Я услышала вас. Сейчас агенты обрабатывают миссию, проверяют данные объекта и готовят объявления.")};r.start()}
$("#voiceBtn").onclick=()=>{if($("#guideStatus").textContent.includes("Слушаю"))return;startGuideVoice()}
